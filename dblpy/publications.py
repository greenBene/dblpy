...
class Publication():
    ...
    def __init__(
            self, authors: List, title: str, venue: str, volume: str,
            number: str, pages: str, publisher: str, year: str, type: str,
            access: str, key: str, doi: str, ee: str, url: str) -> None:
        self.title = title
        self.venue = venue
        self.volume = volume
        self.number = number
    ...
    def __str__(self) -> str:
        s = f'{self.year}. ' + \
            f'{self.authors[0] if self.authors != None else ""} ' + \
            f'et al. "{self.title}". ({self.url})'
        return s
...
def get_publications(q: str, max_results: int = 100) -> List[Publication]:
    ...
    """
    publications_dict = DblpAPI.load_hits(
        endpoint='publ', q=q, max_results=max_results)
